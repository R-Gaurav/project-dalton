...
class DALTON_RTRL_SCNN(torch.nn.Module):
  def __init__(self, rtc):
    ...
    log.INFO("Creating the DALTON RTRL Spiking CNN... Architecture below:")
  ...
  def _forward_through_1_ts(self, x):
    ...
    Args:
      x <Tensor>: Standardized floating point pixel values (all >= 0) for 1 ts.
    """
    all_hdl_1_ts_dltv = (
        self._exu.init_all_hdl_spk_conv_dense_1_ts_delta_v_dict(self))
    all_rol_1_ts_lgts = self._tgu.init_all_rol_1_ts_logits_dict(self)
    ...
    for i, (c_lyr, r_lyr) in enumerate(
      ...
      logits = r_lyr(spikes)
      #all_hdl_1_ts_dltv["conv_lyr_%s" % (i+1)] = delta_v.detach()
      all_hdl_1_ts_dltv["conv_lyr_%s" % (i+1)] = delta_v
      all_rol_1_ts_lgts["conv_lyr_%s" % (i+1)] = logits
    ...
    for i, (d_lyr, r_lyr) in enumerate(
      ...
      logits = r_lyr(spikes)
      #all_hdl_1_ts_dltv["dense_lyr_%s" % (i+1)] = delta_v.detach()
      all_hdl_1_ts_dltv["dense_lyr_%s" %(i+1)] = delta_v
      all_rol_1_ts_lgts["dense_lyr_%s" % (i+1)] = logits

    return all_hdl_1_ts_dltv, all_rol_1_ts_lgts

  def forward(self, x):
    """
    Implements the forward method on the batch input `x` for one time-step.

    Args:
      x <Tensor>: Standardized floating point pixel values (all >= 0) of shape
                  (batch_size, inp_chnls, inp_dmx, inp_dmy) for one time-step.
                  Note that this input would vary for each time-step.
    """
    ...
    # Do forward pass through one time-step.
    all_hdl_1_ts_dltv, all_rol_1_ts_lgts = self._forward_through_1_ts(x)

    if self._rtc.DEBUG: # Check if values w.r.t. all the layers are obtained.
      self._check_values_stored_in_delta_v_and_lgts_dict(
          all_hdl_1_ts_dltv, all_rol_1_ts_lgts)
