...
class TFR_SCNN(torch.nn.Module):
  def __init__(self, rtc):
    ...
    log.INFO("Creating the TFR Spiking CNN... Architecture below:")
  ...
  def _forward_through_time(self, x):
    ...
    all_rol_all_ts_logits = self._tfu.init_all_rol_all_ts_logits_dict()
  ...
  def forward(self, x):
    ...
    if self._rtc.DEBUG: # Check if values w.r.t. all the layers are obtained.
      self._check_values_stored_in_lgts_dict(all_rol_all_ts_logits)
